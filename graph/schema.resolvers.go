...
func (r *mutationResolver) DeleteMeasurement(ctx context.Context, id *string) (*bool, error) {
	user := UseGQLContext(ctx)
	...
	if id == nil {
		return nil, fmt.Errorf("wish ID is required")
	}

	var existingMeasurement models.Measurement
	if err := DB.First(&existingMeasurement, *id).Error; err != nil {
		return nil, fmt.Errorf("measurement not found: %w", err)
	}

	if existingMeasurement.UserID != user.ID {
		return nil, fmt.Errorf("you are not authorized to delete this measuremeny")
	}

	if err := DB.Delete(&existingMeasurement).Error; err != nil {
		return nil, fmt.Errorf("error deleting measuremeny: %w", err)
	}
...
// AddWish is the resolver for the addWish field.
func (r *mutationResolver) AddWish(ctx context.Context, data *model.AddWishInput) (*model.Wish, error) {
	user := UseGQLContext(ctx)
	...
	if err := DB.Create(&newWish).Error; err != nil {
		return nil, fmt.Errorf("error creating user: %w", err)
	}

	response := model.Wish{
		ID:          newWish.ID.String(),
		Name:        newWish.Name,
		URL:         *newWish.Url,
		Description: newWish.Description,
		Image:       newWish.Image,
		Price:       newWish.Price,
		UserID:      string(newWish.UserID.String()),
	}

	return &response, nil
}

// EditWish is the resolver for the editWish field.
func (r *mutationResolver) EditWish(ctx context.Context, data *model.EditUserWishInput) (*model.Wish, error) {
	user := UseGQLContext(ctx)
	...
	var existingWish models.Wish
	if err := DB.First(&existingWish, data.ID).Error; err != nil {
		return nil, fmt.Errorf("wish not found: %w", err)
	}

	// Check if the user is authorized to edit this wish
	if existingWish.UserID != user.ID {
		return nil, fmt.Errorf("you are not authorized to edit this wish")
	}

	// Update the wish fields
	existingWish.Name = data.Name
	if data.URL != nil {
		existingWish.Url = data.URL
	}
	if data.Image != nil {
		existingWish.Image = data.Image
	}
	if data.Description != nil {
		existingWish.Description = data.Description
	}
	if data.Price != nil {
		existingWish.Price = data.Price
	}

	if err := DB.Save(&existingWish).Error; err != nil {
		return nil, fmt.Errorf("error updating wish: %w", err)
	}

	response := model.Wish{
		ID:          existingWish.ID.String(),
		...
		UserID:      string(existingWish.UserID.String()),
	}

	return &response, nil
}

// DeleteWish is the resolver for the deleteWish field.
func (r *mutationResolver) DeleteWish(ctx context.Context, id *string) (*bool, error) {
	user := UseGQLContext(ctx)
	...
	var existingWish models.Wish
	if err := DB.First(&existingWish, *id).Error; err != nil {
		return nil, fmt.Errorf("wish not found: %w", err)
	}

	// Check if the user is authorized to delete this wish
	if existingWish.UserID != user.ID {
		return nil, fmt.Errorf("you are not authorized to delete this wish")
	}

	if err := DB.Delete(&existingWish).Error; err != nil {
		return nil, fmt.Errorf("error deleting wish: %w", err)
	}
